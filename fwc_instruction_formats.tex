...
\hline
42-43 & invert & Bitwise test, jump if not zero (test, jump\_nzero) & dest = src1. Jump if (src1 \& src2) != 0 \\
\hline
44-45 & invert & Bitwise and, jump if not zero (and, jump\_nzero) & dest = src1 \& src2. Jump if dest != 0 \\
\hline
46-47 & invert & Bitwise or, jump if not zero (or, jump\_nzero) & dest = src1 $|$ src2. Jump if dest != 0 \\
\hline
48-49 & invert & Bitwise xor, jump if all selected bits are turned to zero (xor, jump\_test\_and) & dest = src1 \^{} src2. jump if (src1 \& src2) == src2 \\
\hline
...
\hline
56 & 0 & Subtract, jump unconditionally (sub, jump) & dest = src1 + src2. Jump always. \newline No floating point \\
\hline
57 & 1 & Add, jump unconditionally (add, jump) & dest = src1 - src2. Jump always. \newline No floating point \\
\hline
58-59 & 0 jump \newline 1 call & Indirect jump or call with pointer in register RS,
