...
extract       & 1.2 A & 5 & Extract one element from vector RS, starting at offset RT$\cdot$OS, with size OS into scalar in vector register RD. p. \pageref{table:} \\
broad         & 1.2 A & 6 & Broadcast first element of vector RS into all elements of RD with length RT bytes. p. \pageref{table:extractInstruction} \\
...
make\_mask    & 2.6 A & 2 & Make vector where bit 0 of each element comes from bits in IM6, the remaining bits come from RT. p. \pageref{table:} \\
replace       & 2.6 A & 3 & Replace elements in RT by constant IM6. p. \pageref{table:makeMaskInstruction} \\
