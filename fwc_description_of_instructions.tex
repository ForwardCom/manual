...
The signs of the operands can be inverted as indicated by bits 0-3 of the IM3 field in formats 2.4.x and 2.8.x:
\vspace{2mm}
...
This makes it possible to do multiply-and-add, multiply-and-subtract, multiply-and-reverse-subtract, etc. It can also do multiply with alternating add and subtract, which is useful in calculations with complex numbers. There is no sign change in other formats where the IM3 field is absent. 
\vspace{2mm}
...
The signs of the operands can be inverted as indicated by bits 0-2 of the IM3 field in formats 2.4.x and 2.8.x:
\vspace{2mm}
...
There is no sign change in other formats where the IM3 field is absent.
\vspace{2mm}

The precision for floating point operands is preferably better than the least significant bit of the numerically highest operand, but the intermediate result is not necessarily calculated with unlimited precision.
\vspace{2mm}
...
\hline
0.7 0.9        & numeric control register bit 2-5 & numeric control register \\
\hline
...
\vspace{2mm}
Masking and fallback is possible. Alternative use of the fallback register (RU) as an extra boolean operand is supported with at least the following formats: 2.4.6, 2.4.7, 2.8.0, 2.8.6, and 2.8.7. This option  is controlled by bits 4-5 of IM3:
